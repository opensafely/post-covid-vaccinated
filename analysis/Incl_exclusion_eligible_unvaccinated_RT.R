...
input$unvacc_start <- as.Date("2021-06-18")+85
#input$unvacc_start <- as.Date(input$elig_date)+85 - REPLACE & EXECUTE WHEN NEW DEFINITION DATE COME UP
...
str(input$covid_vax_disease_1_date)
...
input$vacc_coh_end_date <- pmin(input$delta_end, input$death_date, 
                               ...
                               input$out_ate, input$out_vte, 
                               input$covid_vax_disease_1_date,
                               input$covid_vax_disease_2_date,
                               input$covid_vax_disease_3_date, na.rm = TRUE)
...
input$prior_infections <- ifelse(input$exp_confirmed_covid19_date < input$unvacc_coh_start_date, 1,0)#1-prior infection; 0 - No prior infection
...
input$prior_vacc1 <- ifelse(input$covid_vax_disease_1_date <= input$unvacc_coh_start_date, 1,0)
input$prior_vacc1[is.na(input$prior_vacc1)] <- 0
input$prior_vacc2 <- ifelse(input$covid_vax_disease_2_date <= input$unvacc_coh_start_date, 1,0)
input$prior_vacc2[is.na(input$prior_vacc2)] <- 0
input$prior_vacc3 <- ifelse(input$covid_vax_disease_3_date <= input$unvacc_coh_start_date, 1,0)
