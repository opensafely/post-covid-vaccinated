...
coxfit <- function(data_surv, interval_names, covar_names, subgroup, mdl){
  ...
  #Base formula
  test_mdl<-"age_sex_no_region_adjusted"
  ...
  for(test_mdl in c("unadjusted","age_adjusted","sex_adjusted","age_sex_no_region_adjusted")){
    ...
    if (test_mdl %in% c("age_adjusted","age_sex_no_region_adjusted") & startsWith(subgroup,"agegp_")==F){
        surv_formula <- paste(surv_formula, "rms::rcs(age,parms=knot_placement)", sep="+")
      }else if (test_mdl %in% c("age_adjusted","age_sex_no_region_adjusted") & (startsWith(subgroup,"agegp_"))==T){
        surv_formula <- paste(surv_formula, "age + age_sq", sep="+")
      }
    
    if (test_mdl %in% c("sex_adjusted","age_sex_no_region_adjusted") & (startsWith(subgroup,"sex"))==F & (!"sex" %in% covariates_excl_region_sex_age)){
