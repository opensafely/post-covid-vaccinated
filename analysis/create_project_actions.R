...
active_analyses <-
  active_analyses %>% filter(active == "TRUE") %>% select(outcome_variable, cohort)
active_analyses$outcome_variable <-
  gsub("out_date_", "", active_analyses$outcome_variable)
cohort_to_run <- c("vaccinated", "electively_unvaccinated")

tmp <-
  crossing(active_analyses[active_analyses$cohort == "all",]$outcome_variable, cohort_to_run)
  ...
  splice(
    ...
    action(
      ...
      moderately_sensitive = list(
        analyses_not_run = glue(
          "output/review/model/analyses_not_run_{outcome}_{cohort}.csv"
        ),
        compiled_hrs_csv = glue(
          "output/review/model/suppressed_compiled_HR_results_{outcome}_{cohort}.csv"
        ),
        compiled_hrs_csv_to_release = glue(
          "output/review/model/suppressed_compiled_HR_results_{outcome}_{cohort}_to_release.csv"
        ),
        compiled_event_counts_csv = glue(
          "output/review/model/suppressed_compiled_event_counts_{outcome}_{cohort}.csv"
        ),
        compiled_event_counts_csv_non_supressed = glue(
          "output/review/model/compiled_event_counts_{outcome}_{cohort}.csv"
        ),
        describe_data_surv = glue(
          "output/not-for-review/describe_data_surv_{outcome}_*_{cohort}_*_time_periods.txt"
        )
      ),
      highly_sensitive = list(
        dataset = glue("output/input_{outcome}_*_{cohort}_*_time_periods.csv"),
        sampled_dataset = glue(
          "output/input_sampled_data_{outcome}_*_{cohort}_*_time_periods.csv"
        )
      )
    )
  )
...
# Updated to a typical action running Cox models for one outcome
apply_model_function_covariate_testing <-
  function(outcome, cohort) {
    splice(
      comment(glue(
        "Cox model {outcome} - {cohort}, covariate_testing"
      )),
      action(
        ...
        needs = list(
          "stage1_data_cleaning_both",
          glue("stage1_end_date_table_{cohort}"),
          glue("stage_2_events_split_by_covariate_level_{cohort}")
        ),
        moderately_sensitive = list(
          analyses_not_run = glue(
            "output/review/model/analyses_not_run_{outcome}_{cohort}_covariate_testing_test_all.csv"
          ),
          compiled_hrs_csv = glue(
            "output/review/model/suppressed_compiled_HR_results_{outcome}_{cohort}_covariate_testing_test_all.csv"
          ),
          compiled_hrs_csv_to_release = glue(
            "output/review/model/suppressed_compiled_HR_results_{outcome}_{cohort}_covariate_testing_test_all_to_release.csv"
          ),
          compiled_event_counts_csv = glue(
            "output/review/model/suppressed_compiled_event_counts_{outcome}_{cohort}_covariate_testing_test_all.csv"
          ),
          compiled_event_counts_csv_non_supressed = glue(
            "output/review/model/compiled_event_counts_{outcome}_{cohort}_covariate_testing_test_all.csv"
          ),
          describe_data_surv = glue(
            "output/not-for-review/describe_data_surv_{outcome}_*_{cohort}_*_covariate_testing_test_all.txt"
          )
        )
      )
    )
...
days_to_event_histogram <- function(cohort) {
  splice(
    comment(glue(
      "Post-exposure days to event histogram data - {cohort}"
    )),
    action(
...
actions_list <- splice(
  ...
  action(
    ...
    highly_sensitive = list(cohort = glue(
      "output/input_electively_unvaccinated.feather"
    ))
  ),
  ...
  action(
    ...
    highly_sensitive = list(cohort = glue("output/input_vaccinated.feather"))
  ),
  ...
  action(
    ...
    highly_sensitive = list(cohort = glue("output/input_index.feather"))
  ),
  ...
  action(
    ...
    moderately_sensitive = list(
      describe = glue(
        "output/not-for-review/describe_input_electively_unvaccinated_*.txt"
      ),
      describe_index = glue(
        "output/not-for-review/describe_tmp_index_electively_unvaccinated.txt"
      ),
      describe_cohort = glue(
        "output/not-for-review/describe_tmp_electively_unvaccinated.txt"
      ),
      descrive_venn = glue(
        "output/not-for-review/describe_venn_electively_unvaccinated.txt"
      )
    ),
    highly_sensitive = list(
  ...
  action(
    ...
    highly_sensitive = list(cohort = glue("output/input_*_stage1.rds"))
  ),
  ...
  splice(# over cohort
    unlist(
      lapply(cohort_to_run, function(x)
        table2(cohort = x)), recursive = FALSE
    )),
  ...
  splice(# over cohort
    unlist(
      lapply(cohort_to_run, function(x)
        days_to_event_histogram(cohort = x)),
      recursive = FALSE
    )),
  ...
  splice(unlist(
    lapply(1:nrow(active_analyses),
           function(i)
             apply_model_function(outcome = active_analyses[i, "outcome_variable"],
  ...
  #Stata cox model
  splice(unlist(
    lapply(1:nrow(analyses_to_run_stata),
           function(i)
             stata_actions(
               outcome = analyses_to_run_stata[i, "outcome"],
  ...
  action(
    ...
    moderately_sensitive = list(
      ...
      R_event_counts_day0 = "output/review/model/R_event_count_day0_output.csv"
    )
  ),
  ...
  action(
    ...
    moderately_sensitive = list(check_table2_cox_event_counts = "output/not-for-review/descriptives/table2_cox_model_event_counts_comparison.csv")
  )
