...
main_estimates$subgroup <- factor(main_estimates$subgroup, levels = c("Age group: 18-39","Age group: 40-59","Age group: 60-79","Age group: 80-110","Sex: Female","Sex: Male",
...
for (outcome_name in unique(main_estimates$outcome)) {
  if(grepl("Primary position",outcome_name)){
    tmp_reduced <- main_estimates %>% filter(time_points == "reduced" 
                                     & outcome %in% outcome[grepl("Primary position",outcome)]
                                     & outcome == outcome_name)
    tmp_normal <- main_estimates %>% filter(time_points == "normal" 
                                            & outcome %in% outcome[grepl("Primary position",outcome)]
                                            & outcome == outcome_name)
  }else{
    tmp_reduced <- main_estimates %>% filter(time_points == "reduced" 
