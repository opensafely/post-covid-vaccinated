...
time_points_of_interest="normal"
  ...
  for(outcome_position in c("any_position","primary_position")){
    for (time_points_of_interest in c("reduced","normal")) {
      ...
      if(outcome_position == "any_position"){
        df=lifetables_main %>% filter(cohort == cohort_of_interest & !str_detect(event, "primary_position") & time_points == time_points_of_interest)
      }else{
        df=lifetables_main %>% filter(cohort == cohort_of_interest & str_detect(event, "primary_position") & time_points == time_points_of_interest)
      }
      ...
      if(nrow(df)>0){
        ...
        ggplot2::ggplot(data = df, 
          ...
          ggplot2::ggtitle(paste0(cohort_of_interest, " cohort; ", outcome_position, " outcomes; ", time_points_of_interest, " time periods; overall HRs")) +
        ...
        ggsave(paste0(aer_output_dir, "/figure_4_",cohort_of_interest,"_",outcome_position,"_", time_points_of_interest, "_time_periods_using_overall_HRs.png"), height = 210, width = 297, unit = "mm", dpi = 600, scale = 1)
...
for(cohort_of_interest in cohort_name){
  for(outcome_position in c("any_position","primary_position")){
    for (time_points_of_interest in c("reduced","normal")) {
      ...
      if(outcome_position == "any_position"){
        df=lifetables_subgroup %>% filter(cohort == cohort_of_interest & !str_detect(event, "primary_position") & time_points == time_points_of_interest )
      }else{
        df=lifetables_subgroup %>% filter(cohort == cohort_of_interest & str_detect(event, "primary_position") & time_points == time_points_of_interest )
      }
      ...
      if(nrow(df)>0){
        ...
        ggplot2::ggplot(data = df, 
                        mapping = ggplot2::aes(x = days, y = excess_risk_subgroup, color = agegroup, shape = agegroup, fill = agegroup, linetype = sex)) +
          #ggplot2::geom_hline(colour = "#A9A9A9") +
          #geom_ribbon(aes(ymin = CIp.low, ymax = CIp.high), alpha = 0.1)+
          ggplot2::geom_line() +
          ggplot2::scale_x_continuous(lim = c(0,round_any(max(df$days, na.rm = T),20, f= ceiling)), breaks = seq(0,round_any(max(df$days, na.rm = T),20, f= ceiling),20)) +
