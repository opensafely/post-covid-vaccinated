...
events <- active_analyses %>% filter(active=="TRUE")%>%select(outcome,outcome_variable, outcome_group)
...
combined_hr <- combined_hr %>% filter(subgroup %in% c("covid_pheno_non_hospitalised"))
...
for(i in cohort){
  for (j in unique(combined_hr$outcome_group)) {
    # Filter to cohort of interest
    
    df=combined_hr %>% filter(cohort ==i  & outcome_group == j)
