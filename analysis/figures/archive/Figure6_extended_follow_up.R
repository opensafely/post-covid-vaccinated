...
outcome_name_table <- active_analyses %>% 
  ...
  mutate(outcome_name=active_analyses$outcome_variable %>% str_replace("out_date_", ""),
         outcome_name = paste0(outcome_name,"_extended_follow_up"))
...
estimates$colour <- ifelse(estimates$subgroup=="main","#000000",estimates$colour)
estimates$colour <- ifelse(estimates$subgroup=="covid_pheno_hospitalised","#e31a1c",estimates$colour)
estimates$colour <- ifelse(estimates$subgroup=="covid_pheno_non_hospitalised","#fb9a99",estimates$colour)

# Specify subgorup name
estimates$subgroup <- ifelse(estimates$subgroup=="main", "Maximally adjusted")
estimates$subgroup <- ifelse(estimates$subgroup=="covid_pheno_hospitalised", "Hospitalised COVID-19")
estimates$subgroup <- ifelse(estimates$subgroup=="covid_pheno_non_hospitalised", "Non-hospitalised COVID-19")

# Factor variables for ordering
estimates$subgroup <- factor(estimates$subgroup, levels=c("Maximally adjusted","Hospitalised COVID-19","Non-hospitalised COVID-19")) 
estimates$colour <- factor(estimates$colour, levels=c("#000000","#e31a1c","#fb9a99"))
...
for(i in c("any_position","primary_position")){
  ...
  ggplot2::ggsave(paste0(output_dir,"Figure_6_extended_follow_up_",i,"_events.png"), height = 297, width = 210, unit = "mm", dpi = 600, scale = 1)
