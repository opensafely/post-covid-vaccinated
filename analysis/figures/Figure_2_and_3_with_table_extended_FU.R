...
outcome_name_table <- active_analyses %>% 
  ...
  #filter(outcome_name %in% c("ate","vte"))
  filter(outcome_name %in% c("ate","vte","ate_primary_position","vte_primary_position"))

#outcomes_to_plot <- outcome_name_table$outcome_name[outcome_name_table$outcome_name %in% c("ate","vte")]
#outcomes_to_plot_prevax <- c("ate_extended_follow_up","vte_extended_follow_up")
...
outcomes_to_plot_prevax <- c("ate_extended_follow_up","vte_extended_follow_up","ate_primary_position_extended_follow_up","vte_primary_position_extended_follow_up")

outcome_name_table$outcome <- gsub(" events","",outcome_name_table$outcome)

# Load all estimates
estimates <- read.csv(paste0(results_dir,"/hr_output_formatted.csv")) # 7148 rows

# CHECK Stata and R files
# estimates_R <- read.csv(paste0(results_dir,"/R_HR_output.csv")) # 16497 rows
# estimates_Stata <- read.csv(paste0(results_dir,"/stata_output.csv")) # 3775 rows
# estimates_R_prevax <- read.csv(paste0(results_dir,"/R_HR_output_pre_vax.csv")) # 23035 rows
# estimates_Stata_prevax <- read.csv(paste0(results_dir,"/stata_output_pre_vax.csv")) # 4532 rows

# Get estimates for main analyses and list of outcomes from active analyses
estimates_delta <- estimates %>% filter(subgroup %in% c("covid_pheno_non_hospitalised","covid_pheno_hospitalised", "main")
                                       & cohort %in% c("vaccinated","electively_unvaccinated")
                                       #& time_points %in% c("normal","reduced")
                                       & event %in% outcomes_to_plot 
                                       & term %in% term[grepl("^days",term)]
                                       & model == "mdl_max_adj") %>%
  select(term,estimate,conf_low,conf_high,event,subgroup,cohort,time_points,median_follow_up)

estimates_prevax <- estimates %>% filter(subgroup %in% c("covid_pheno_non_hospitalised","covid_pheno_hospitalised", "main")
...
xbreaks <- seq(0,round_any(max(estimates$median_follow_up, na.rm = T),4, f= ceiling),8)
...
for(i in outcome_name_table$outcome){
  ...
  # -> Here change size=9 to size=9.8
  ...
  png(paste0(output_dir,"Figure_2_with_table_",short_outcome,"_extended_FU.png"),
