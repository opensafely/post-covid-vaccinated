...
# Currently based on end of June results for vax, electively unvax.
# -> We will need to add prevax here when results become available.
...
# -> We now want to have all 10 outcomes
# Remove outcomes called primary position
outcomes_to_exclude <- outcome_name_table$outcome_name[grep("primary_position", outcome_name_table$outcome_name)]
outcomes_to_plot <- outcome_name_table$outcome_name [outcome_name_table$outcome_name != outcomes_to_exclude]

#---------------------------------------------#
# 3. Load and combine all estimates in 1 file #
#---------------------------------------------#
hr_files=list.files(path = results_dir, pattern = "suppressed_compiled_HR_results")
...
main_estimates <- subset(estimates, subgroup == "main" & event %in% outcomes_to_plot & term %in% term[grepl("^days",term)])
...
main_estimates$colour <- ifelse(main_estimates$cohort=="vaccinated","#bababa",main_estimates$colour) # Grey
main_estimates$colour <- ifelse(main_estimates$cohort=="electively_unvaccinated","#000000",main_estimates$colour) # Black

# Factor variables for ordering
#main_estimates$model <- factor(main_estimates$model, levels=c("mdl_agesex", "mdl_max_adj")) 
main_estimates$cohort <- factor(main_estimates$cohort, levels=c("vaccinated","electively_unvaccinated")) 
...
levels(main_estimates$cohort) <- list("Vaccinated cohort"="vaccinated","Electively unvaccinated cohort"="electively_unvaccinated")
...
#  df=main_estimates %>% filter(cohort ==c)
  ...
  # Need to remove extreme estimates and convert estimate and CI into numeric
  df=main_estimates %>% filter(model == "mdl_max_adj" & robust.se != "Inf")
  ...
  ggplot2::ggsave(paste0(output_dir,"/Figure1_by_cohort.png"), height = 297, width = 210, unit = "mm", dpi = 600, scale = 1)
#}