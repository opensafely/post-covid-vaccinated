...
median_follow_up_pre_vax <- median_follow_up %>% filter(cohort == "vaccinated" & median_follow_up != "[Redacted]")
median_follow_up_pre_vax$cohort <- "pre_vax"
median_follow_up <- rbind(median_follow_up,median_follow_up_pre_vax)
median_follow_up <- na.omit(median_follow_up)
...
main_estimates <- main_estimates %>% filter(model %in% c("mdl_age_sex","mdl_max_adj") & ((event == "ate" & term %in% c("days0_28","days28_197")) |(event == "vte" & term %in% c("days0_7","days7_14", "days14_28", "days28_56", "days56_84", "days84_197") & cohort == "vaccinated") |(event == "vte" & term %in% c("days0_28","days28_197") & cohort == "electively_unvaccinated")))
main_estimates$expo_week <- NULL
main_estimates$median_follow_up <- NULL


#Add in dummy pre-vax results for plotting
pre_vax <- main_estimates %>% filter(cohort == "vaccinated")
pre_vax$cohort <- "pre_vax"

main_estimates <- rbind(main_estimates,pre_vax)
main_estimates$time_points <- ifelse(is.na(main_estimates$time_points),"normal",main_estimates$time_points)
#--------------------------------------#
...
main_estimates$cohort <- factor(main_estimates$cohort, levels=c("pre_vax", "vaccinated","electively_unvaccinated"))
...
levels(main_estimates$cohort) <- list("Pre-vaccinated"="pre_vax", "Vaccinated"="vaccinated", "Electively unvaccinated"="electively_unvaccinated")
