...
#--------------------------------------#
# Select estimates for time periods only
main_estimates <- main_estimates %>% dplyr::filter(str_detect(term, "^days"))
...
main_estimates$colour <- ifelse(main_estimates$model=="mdl_agesex","#000000",main_estimates$colour) # Black
main_estimates$colour <- ifelse(main_estimates$model=="mdl_max_adj","#bababa",main_estimates$colour) # Grey

# Factor variables for ordering
main_estimates$model <- factor(main_estimates$model, levels=c("mdl_agesex", "mdl_max_adj")) 
main_estimates$colour <- factor(main_estimates$colour, levels=c("#000000", "#bababa"))
...
#   ggplot2::scale_y_continuous(lim = c(0.25,64), breaks = c(0.5,1,2,4,8,16,32,64), trans = "log") +
    ...
    ggplot2::scale_shape_manual(values = c(rep(21,18)), labels = levels(df$model)) +
