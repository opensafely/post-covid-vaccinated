...
for(event in active_analyses$outcome_variable){
  ...
  if(cohort=="vaccinated"){
    input$follow_up_end_unexposed <- apply(input[,c("event_date", "exp_date_covid19_confirmed", "death_date", "cohort_end_date")],1, min,na.rm=TRUE)
    ...
    input$follow_up_end_unexposed <- apply(input[,c("vax_date_covid_1","event_date", "exp_date_covid19_confirmed", "death_date","cohort_end_date")],1, min,na.rm=TRUE)
