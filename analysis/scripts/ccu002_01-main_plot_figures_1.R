...
df <- data.table::fread("output/compiled_HR_results_subgroup_covid_pheno_vaccinated_delta_mdl_max_adj_covid_history_false.csv", 
                        data.table = FALSE)
...
ggplot2::ggplot(data = df[df$stratification=="Overall",],
  ...
  ggplot2::scale_fill_manual(values = levels(df$colour)[1:2], labels = levels(df$stratum)[1:2]) +
  ggplot2::scale_color_manual(values = levels(df$colour)[1:2], labels = levels(df$stratum)[1:2]) +
  ggplot2::scale_shape_manual(values = c(rep(c(21,22),3),23,24)[1:2], labels = levels(df$stratum)[1:2]) +
...
ggplot2::ggsave(paste0("output/ccu002_01_main_figure1.png"), height = 297, width = 210, unit = "mm", dpi = 600, scale = 1)


ggplot2::ggplot(data = df[df$stratification=="Hospitalised/Non-hospitalised COVID-19",],
                mapping = ggplot2::aes(x = time, y = estimate, color = stratum, shape=stratum, fill=stratum)) +
  ggplot2::geom_hline(mapping = ggplot2::aes(yintercept = 1), colour = "#A9A9A9") +
  ggplot2::geom_point(position = ggplot2::position_dodge(width = 1)) +
  ggplot2::geom_errorbar(mapping = ggplot2::aes(ymin = ifelse(conf.low<0.25,0.25,conf.low), 
  ...
  ggplot2::scale_y_continuous(lim = c(0.25,64), breaks = c(0.5,0.5,1,2,4,8,16,32,64), trans = "log") +
  ggplot2::scale_x_continuous(lim = c(0,44), breaks = seq(0,44,4)) +
  ggplot2::scale_fill_manual(values = levels(df$colour)[3:4], labels = levels(df$stratum)[3:4]) +
  ggplot2::scale_color_manual(values = levels(df$colour)[3:4], labels = levels(df$stratum)[3:4]) +
  ggplot2::scale_shape_manual(values = c(rep(c(21,22),3),23,24)[3:4], labels = levels(df$stratum)[3:4]) +
  ggplot2::labs(x = "\nWeeks since COVID-19 diagnosis", y = "Hazard ratio and 95% confidence interval") +
  ggplot2::guides(fill=ggplot2::guide_legend(ncol = 4, byrow = TRUE)) +
  ggplot2::theme_minimal() +
  ggplot2::theme(panel.grid.major.x = ggplot2::element_blank(),
...
ggplot2::ggsave(paste0("output/ccu002_01_suppl_figure1.png"), height = 297, width = 210, unit = "mm", dpi = 600, scale = 1)
